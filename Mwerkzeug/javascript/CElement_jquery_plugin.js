(function ($) {
  ...
  var methods = {
    ...
    child_edit: function (element, args) {
      return methods.edit(element, '', args);
    },
    edit: function (element, urlAddon, args) {
      ...
      element.load(url, params, function () {
        ...
        var size = 0;
        var settings = $(element).closest('.CElementList').CElement('getSettings');
        var allowed_CTypes = settings.allowed_CTypes;

        if (args.child_id) // no Ctypes for children
        {
          allowed_CTypes = {};
        }

        if (allowed_CTypes) {
          size = Object.size(allowed_CTypes);
        }
        ...
        if (size < 2) {
          $('.celement-ctypeswitcher', element).parent().hide();
        }

        $('a.celement_submit', element).on('click', function (e) {
          e.preventDefault();
          CElement_submit(this);
        });

        $('a.celement_cancel', element).on('click', function (e) {
          e.preventDefault();
          CElement_cancel(this);
        });

        $.each(allowed_CTypes, function (key, value) {
          $('.celement-ctypeswitcher .dropdown-menu', element).append("<li><a href='#" + key + "' class='active'>" + value + "</a></li>");
        });

        $('.celement-ctypeswitcher', element).MwButtonDropdown().on('btn_change', function (e, data) {

          $(this).append("<input type='hidden' name='nextaction' value='edit'>");
          CElement_submit(this);
        });
    ...
    duplicate: function (element) {
      ...
      if (confirm('are you sure ?')) {
        methods.additem.apply(this, [{
          'duplicateOf': element.attr('id'),
          'insertAfter': element
        }]);
      }
    },
    ...
    list: function () {
      ...
      this.load(url, params, function () {
        ...
        $(this).on('click', '.c4p-child-editbutton', function (e) {
          e.preventDefault();
          var child_id = $(this).attr('href').replace('#', '');
          if ($(this).closest('.CElement').hasClass('editmode')) {
            $(this).closest('form').append("<input type='hidden' name='nextaction' value='child_edit'>");

            $(this).closest('form').append("<input type='hidden' name='nextaction_jsonargs' value='{\"child_id\":\"" + child_id + "\"}'>");
            CElement_submit(this);
          } else {

            methods.child_edit($(this).closest('.celement-li'), {
              'child_id': child_id
            });
    ...
    copySelectionToClipboard: function (self, func_args) {
      ...
      if (element) {
        ...
        $.post(url, params, function (response) {
          ...
          $('.CElementCopyPasteControlFeedback', self).delay(2200).slideUp('slow', function () {
            $(this).remove();
          });
    ...
    checkHiddenButton: function (self) {
      if (window.console && console.log) {
        console.log('checkHiddenButton go', null);
      }
      if ($(this).is(":checked")) {
        $('li > .c4p-hidden', self).show();
      } else {
        $('li > .c4p-hidden', self).hide();
      }
    ...
    checkHidden: function () {
      ...
      //count hidden
      var hidecount = $('li > .c4p-hidden', this).length;
      if (hidecount > 0) {
        if ($('.CElementHideCb', this).length == 0) {

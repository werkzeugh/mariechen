...
// mwuits ---------- BEGIN
if(strstr(array_get($_SERVER,'QUERY_STRING'),','))
{
    if(preg_match('#^(\d+),(\d+),(\d+),([0-9.]+)$#',array_get($_SERVER,'QUERY_STRING'),$m))
    {
        array_get($_REQUEST,'r')=$m[1];
        array_get($_REQUEST,'g')=$m[2];
        array_get($_REQUEST,'b')=$m[3];
        array_get($_REQUEST,'a')=$m[4];
    }
}
...
$rgb = $color_names[array_get($_REQUEST,'name')];

$red	= is_array($rgb)? $rgb[0] : intval(array_get($_REQUEST,'r'));
$green	= is_array($rgb)? $rgb[1] : intval(array_get($_REQUEST,'g'));
$blue	= is_array($rgb)? $rgb[2] : intval(array_get($_REQUEST,'b'));

// "A value between 0 and 127. 0 indicates completely opaque while 127 indicates completely transparent."
// http://www.php.net/manual/en/function.imagecolorallocatealpha.php
$alpha = intval(127 - 127 * (array_get($_REQUEST,'a') ));

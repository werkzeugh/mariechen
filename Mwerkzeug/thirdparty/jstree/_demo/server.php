...
if((array_get($_GET,'reconstruct'))) {
	$jstree->_reconstruct();
	die();
}
if((array_get($_GET,'analyze'))) {
	echo $jstree->_analyze();
	die();
}

if(array_get($_REQUEST,'operation') && strpos(array_get($_REQUEST,'operation'), "_") !== 0 && method_exists($jstree, array_get($_REQUEST,'operation'))) {
	header("HTTP/1.0 200 OK");
	header('Content-type: application/json; charset=utf-8');
	header("Cache-Control: no-cache, must-revalidate");
	header("Expires: Mon, 26 Jul 1997 05:00:00 GMT");
	header("Pragma: no-cache");
	echo $jstree->{array_get($_REQUEST,'operation')}($_REQUEST);
	die();
}

 ...
 * 
 * Usage through URL: http://localhost/dev/tasks/MwStaticTextCollectorTask
 * Usage through URL (module-specific): http://localhost/dev/tasks/MwStaticTextCollectorTask/?module=mymodule
 * Usage on CLI: sake dev/tasks/MwStaticTextCollectorTask
 * Usage on CLI (module-specific): sake dev/tasks/MwStaticTextCollectorTask module=mymodule
 *
...
class MwStaticTextCollector  {
	...
	public function run($restrictToModules = null) {
        ...
        //alle entities mit priorität=50 entfernen (standard-kram von silverstripe)
        ...
        foreach ($entitiesByModule as $module => $entities) {
            ...
            foreach ($entities as $key=>$value) {
                if($value[1]=='50')
                {
                    unset($entitiesByModule[$module][$key]);
                }
            }
        }

        echo MwBackendPageController::viewArray($entitiesByModule);

		// Write the generated master string tables
		// $this->writeMasterStringFile($entitiesByModule);
		
		Debug::message("Done!", false); flush();
	}
	...
	protected function processModule($module) {	
		...
		Debug::message("Processing Module '{$module}'", false); flush();
        ...
        if(array_get($_GET,'d') || 1 ) { $x=$entitiesArr; $x=htmlspecialchars(print_r($x,1));echo "\n<li>ArrayList: <pre>$x</pre>"; }
	...
	function collectFromEntityProviders($filePath) {
		...
		if($classes) foreach($classes as $class) {
			...
			// Requires PHP 5.1+
			if(class_exists($class) && in_array(i18nEntityProvider::class, class_implements($class))) {
	...
	public function langArrayCodeForEntitySpec($entityFullName, $entitySpec) {
		...
		if(count($entityParts) > 1) {
			...
			user_error("MwStaticTextCollector::langArrayCodeForEntitySpec(): Wrong entity format for $entityFullName with values" . var_export($entitySpec, true), E_USER_WARNING);
			return false;
		}
	...
	protected function writeMasterStringFile($entitiesByModule) {
        ...
        echo "<li>  writeMasterStringFile...";flush();
		if($entitiesByModule) foreach($entitiesByModule as $module => $entities) {
			...
			if(!file_exists($langFolder)) {
				Filesystem::makeFolder($langFolder, 02775);
			...
			if($fh = fopen($langFile, "w")) {
				...
				try{
					...
					user_error('MwStaticTextCollector->writeMasterStringFile(): Invalid PHP language file. Error: ' . $e->toString(), E_USER_ERROR);
				}
				
				fwrite($fh, "<"."?php{$eol}{$eol}global \$lang;{$eol}{$eol}" . $php . "{$eol}");
				fclose($fh);
				
				Debug::message("Created file: $langFolder/" . $this->defaultLocale . ".php", false); flush();
			} else {

	...
	function __construct($data) {
		...
		if ($this->config['refererMustMatch']) {
			foreach($this->config['allowedRefererHosts'] as $i => $host)
				if ($host == '.')
					$this->config['allowedRefererHosts'][$i] = array_get($_SERVER,'HTTP_HOST');
			$referer = parse_url(array_get($_SERVER,'HTTP_REFERER'), PHP_URL_HOST);
			$authorized = $referer
				&& in_array($referer, $this->config['allowedRefererHosts']);
			if (! $authorized) {
				throw new Exception("Host '{array_get($_SERVER,'HTTP_REFERER')}' not authorized to make requests.");
				return;
			}
		}
...
new jQueryServer(array_get($_POST,'data'));
?>
...
class ArrayToXml {
	...
	public function __construct(array $input, $maxDepth = 10, $trim = true) {
		...
		if (count($input) == 1) {
			$tagName = key($input);
			$Tag = new MwSofortTag($tagName, $this->_extractAttributesSection($input[$tagName]), $this->_extractDataSection($input[$tagName], $trim));
	...
	private function _createNode($name, array $attributes, array $children) {
		return new MwSofortTag($name, $attributes, $children);
	}
	...
	private function _createTextNode($text, $trim) {
		return new MwSofortText($text, true, $trim);
	}
